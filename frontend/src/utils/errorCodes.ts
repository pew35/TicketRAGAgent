// Application error codes mirrored from server/server/web/api/error_codes.py.
...
export enum ErrorCode {
  ...
  // External HTTP and service errors: 1500-1599
  ...
  // Agent and RAG errors: 2000-2099
  AGENT_ERROR = 2000,
...
// Return the matching default message or the unknown-error fallback.
...
// Return whether the response represents a successful operation.
export function isSuccess(code: ErrorCode | number): boolean {
  return code === ErrorCode.SUCCESS;
}

// Return whether the code belongs to request or parameter validation errors.
export function isParamError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1000, 1099);
}

// Return whether the code belongs to authentication or authorization errors.
export function isAuthError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1100, 1199);
}

// Return whether the code belongs to user, conversation, or message business errors.
export function isBusinessError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1200, 1399);
}

// Return whether the code belongs to database or cache infrastructure errors.
export function isDatabaseError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1400, 1499);
}

// Return whether the code belongs to external HTTP or service errors.
export function isExternalServiceError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1500, 1599);
}

// Return whether the code belongs to file or upload errors.
export function isFileError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1600, 1699);
}

// Return whether the code belongs to internal server errors.
export function isInternalError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 1900, 1999);
}

// Return whether the code belongs to Agent or RAG processing errors.
export function isAgentError(code: ErrorCode | number): boolean {
  return isCodeInRange(code, 2000, 2099);
}

// Return whether the numeric value is defined by the shared error-code contract.
export function isKnownErrorCode(code: ErrorCode | number): boolean {
  return Object.prototype.hasOwnProperty.call(ERROR_MESSAGES, code);
}

function isCodeInRange(code: ErrorCode | number, start: number, end: number): boolean {
